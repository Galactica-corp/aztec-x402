...
export class TokenContract extends ContractBase {
   ...
   */
  public static deploy(wallet: Wallet, name: string, symbol: string, decimals: (bigint | number), initial_supply: (bigint | number), to: AztecAddressLike, auth_contract: AztecAddressLike, instantiation?: DeployInstantiationOptions) {
    return DeployMethod.create<TokenContract>(
      wallet,
      {
        artifact: TokenContractArtifact,
        postDeployCtor: (instance, wallet) => TokenContract.at(instance.address, wallet),
        args: [name, symbol, decimals, initial_supply, to, auth_contract],
      },
      instantiation,
    );
  }
  ...
  public static get storage(): ContractStorageLayout<'name' | 'symbol' | 'decimals' | 'private_balances' | 'total_supply' | 'public_balances' | 'minter' | 'auth_contract'> {
      return {
        name: {
      slot: new Fr(1n),
    },
...
auth_contract: {
      ...
      } as ContractStorageLayout<'name' | 'symbol' | 'decimals' | 'private_balances' | 'total_supply' | 'public_balances' | 'minter' | 'auth_contract'>;
    }
  ...
  public declare methods: {
    ...
    /** constructor_with_initial_supply(name: string, symbol: string, decimals: integer, initial_supply: integer, to: struct, auth_contract: struct) */
    constructor_with_initial_supply: ((name: string, symbol: string, decimals: (bigint | number), initial_supply: (bigint | number), to: AztecAddressLike, auth_contract: AztecAddressLike) => ContractFunctionInteraction) & Pick<ContractMethod, 'selector'>;

    /** constructor_with_minter(name: string, symbol: string, decimals: integer, minter: struct, auth_contract: struct) */
    constructor_with_minter: ((name: string, symbol: string, decimals: (bigint | number), minter: AztecAddressLike, auth_contract: AztecAddressLike) => ContractFunctionInteraction) & Pick<ContractMethod, 'selector'>;

    /** decimals() */
    decimals: (() => ContractFunctionInteraction) & Pick<ContractMethod, 'selector'>;

    /** get_auth_contract() */
    get_auth_contract: (() => ContractFunctionInteraction) & Pick<ContractMethod, 'selector'>;
    ...
    "fields": [
        {
            ...
            "type": {
                ...
                "path": "authorization_contract::aztec::protocol_types::address::aztec_address::AztecAddress"
            }
        },
        {
            "name": "to",
